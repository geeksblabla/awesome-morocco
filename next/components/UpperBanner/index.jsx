...
import styles from "../../styles/UpperBanner.module.scss";

const UpperBanner = () => {
  const [showUpperBanner, setShowUpperBanner] = React.useState(true);

  React.useEffect(() => {
    const  data = window.localStorage.getItem('UPPER_BANNER_INFO');
    if ( data !== null ) setShowUpperBanner(JSON.parse(data));
  }, [])

  React.useEffect(() => {
    window.localStorage.setItem('UPPER_BANNER_INFO', JSON.stringify(showUpperBanner));
  }, [showUpperBanner])
  ...
  return (
    ...
    { showUpperBanner && (
      <div className={styles.upperbanner}>
        <p>
          This website is part of <a href="https://geeksblabla.com/" target="_blanc">the geeksblabla</a> open source projects
        </p>
        <button onClick={() => setShowUpperBanner(false)}>
          <svg
            width="10"
            height="10"
            viewBox="0 0 9 10"
